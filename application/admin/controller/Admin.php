...
class Admin extends Controller {
     ...
     */
    //批量修改删除
    public function changeSent1($method=null,$model=null,$type="status"){
        $data=input('sid/a');
    ...
    /* @param string $model 模型名称,供D函数使用的参数*/
    //状态修改
    public function setproperty1($model) {
        $id = input('sid');
        $type = input('type');
        $data = input('data');;
        if ($type) {
            $data = ($data==1?'0':'1');
            \think\Db::name($model)->where(array("sid"=>$id))->update(array($type=>$data));
            exit(json_encode(array("result" => 1, "data" => $data)));
        }
        die(json_encode(array("result" => 0)));
    }
