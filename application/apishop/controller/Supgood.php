...
class Supgood extends Supbase
{
   ...
   public function editgood(){
       ...
       $data['goods_option']=$_POST["goods_option"];//商品规格
    ...
    public function changegood(){
        ...
        $data['title']=$_POST["title"]?$_POST["title"]:"";//标题
        $data['band']=$_POST["band"]?$_POST["band"]:"";//品牌
        $data['is_show']=$_POST["show"]?$_POST["show"]:$good['is_show'];//上架:1上架，2:下架
        $data['is_delete']=$_POST["del"]?$_POST["del"]:$good['is_delete'];//删除:1未删除，2：已删除
        $data['price']=$_POST["price"]?$_POST["price"]:"";//价格
        $data['catid']=$_POST["catid"]?$_POST["catid"]:"";//分类id
        $data['s_sort']=intval($_POST["s_sort"])?$_POST["s_sort"]:"";//店铺内排序
        $data['photo_x']=$_POST["photo_x"]?$_POST["photo_x"]:"";//图片
        if(!$data['photo_x']){
            _internalError('商品图片必传','500','');
        }
        $data['photo_x']=str_replace(__DATAURL__, "", $data['photo_x']);

        $data['detail']=$_POST["detail"]?$_POST["detail"]:"";//简要描述
        $data['content']=$_POST["content"]?$_POST["content"]:"";//详细描述
        $data['goods_option']=$_POST["goods_option"]?$_POST["goods_option"]:"";//商品规格
        $data['photo_string']=$_POST["photo_string"]?$_POST["photo_string"]:"";//详细图片
        $data['photo_string']=str_replace(__DATAURL__, "", $data['photo_string']);
