...
class AuthGroup extends Model
{
    ...
    public function addToGroup($uid, $gid)
    {
        ...
        foreach ($uid_arr as $u) {
            ...
            //判断用户id是否合法
            if (\think\Db::name('ucenter_member')->getFieldByUid($u, 'uid') == false) {
     ...
     *  array(2,4,8,13)
     */
    public static function getAuthExtend($uid, $type, $session)
    {
        if (!$type) {
            return false;
        }
     ...
     *  array(2,4,8,13)
     */
    public static function getAuthCategories($uid)
    {
     ...
     *  array(2,4,8,13)
     */
    public static function getExtendOfGroup($gid, $type)
    {
        if (!is_numeric($type)) {
            return false;
        }
     ...
     *  array(2,4,8,13)
     */
    public static function getCategoryOfGroup($gid)
    {
     ...
     */
    public static function addToExtend($gid, $cid, $type)
    {
     ...
     * @param int|string|array $cid   分类id
     */
    public static function addToCategory($gid, $cid)
    {
     ...
     */
    public static function memberInGroup($group_id)
    {
        $prefix  = config('database.prefix');
        $l_table = $prefix . self::ucenter_member;
