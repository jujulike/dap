...
function  addons_hook($name,$field = true){
	$data = \think\Db::name('Hooks')->cache(true)->field($field)->where(['addons'=>['like','%'.$name.'%']])->find();
	return $data;
}
...
function sendordermessages($uid,$ordersn,$message,$url){
    $mes['type']='2';
    $mes['orderid']=$ordersn;
    $mes['content']=$message;
    $mes['uid']=$uid;
    $mes['url']=$url;
    $mes['addtime']=time();
    $inmess=db('message')->insertGetId($mes);
}
