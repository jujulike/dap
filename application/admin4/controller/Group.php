...
class Group extends Admin
{
    public function _initialize()
    {
        ...
        $orders = Db::name("shop_order")->where(['sent' => 2, 'pay_status' => 1, 'expresstime' => array('lt', time())])->column("id");
    ...
    public function order()
    {
        ...
        if ($btns == 'explode' || $btns == 'explodegood') {
            ...
            $list = $this->lists('shop_order', $map, 'id desc');
            foreach ($list as &$v) {
    ...
    public function order_info()
    {
        ...
        if (request()->isPost()) {
            ...
            $order_info = Db::name('shop_order')->where(['id' => $did])->find();

            $addr = explode("|", $order_info['address']);
            $this->assign('addr', $addr);

            $dispatch = Db::name("dispatch")->where('id', $order_info['dispatchid'])->find();
            $goods    = Db::name("shop_order_good")->where(['order_id' => $order_info['id']])->select();
            foreach ($goods as &$v) {
    ...
    {
        $did   = input('id');
        $order = Db::name('shop_order')->where(array('id' => $did))->find();
        if ($order['pay_status'] && $order['sent'] == -1 && empty($order['isrefund'])) {
            ...
            if ($res) {
                ...
                if ($order['istypes'] == 1) {
                    ...
                    $good = Db::name("shop_order_good")->where(['order_id' => $did])->select();
                    foreach ($good as $v) {
                        if ($v['optionid']) {
    ...
    {
        $did = input('id');
        $res = Db::name('shop_order')->where(array('id' => $did))->count();
        if ($res) {
            action_log('del_shop_order', 'shop_order', $did, UID); //记录行为
            Db::name('shop_order')->where(array('id' => $did))->update(['is_delete' => 1]);
            $this->success("删除成功");
        } else {
            $this->error("订单存在已经删除");
        }
    ...
    {
        $did         = input('id');
        $couponprice = Db::name("shop_order")->where("id", $did)->value("couponprice");
        ...
        Db::name("shop_order")->where("id", $did)->update(['price' => $price, 'goodsprice' => $goodsprice, 'mailprice' => $mailprice]);
        $this->success('修改价格成功');
    }
}
