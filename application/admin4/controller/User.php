...
class User extends Admin{
    ...
    public function submitPassword(){
        ...
        $Api    =   model(UcenterMember);
        $res    =   $Api->updateUserFields(UID, $password, $data);
        if($res['status']){
            $this->success('修改密码成功！');
        }else{
            $this->error($res['info']);
    ...
    public function changeStatus($method=null){  
        ...
        switch ( strtolower($method) ){
            case 'forbiduser':
                $this->forbid('ucenterMember', $map,'status' );
                break;
            case 'resumeuser':
                $this->resume('ucenterMember', $map,'status' );
                break;
            case 'deleteuser':
                $this->deleteids('ucenterMember', $map );
                break;
            default:
                $this->error('参数非法');
        }
