...
// 常量定义
define('THINKPHP_VERSION', '1.0');
define('THINKPHP_ADDON_PATH', ROOT_PATH.'/addons/');
...
function get_nickname($uid = 0)
{
    ...
    if (isset($list[$key])) { //已缓存，直接使用
        ...
        $info = db('Ucenter_member')->field('username')->find($uid);
 ...
 * 获取商品首图
 */
function getGoodPic($gid = '')
{
    $data = '/zhiing/admin/img/webuploader.png';
    $data = db('good')->where('id', $gid)->value('photo_x');
    if ($data) {
        $img = explode(',', $data);
        $data = $img[0];
    }
 ...
 * 获取店铺类目
 */
function getShopCate($sid = '')
{
    $data = '';
    $type = db('supuser')->where('sid', $sid)->value('type');
    if ($type) {
        $where['is_delete'] = ['eq', 0];
        $where['id'] = ['in', $type];
        $category = db('category')->where($where)->column('catename');
 ...
 * @param int $id 分类ID
 *
 * @return bool true-允许发布内容，false-不允许发布内容
 */
 ...
 *
 * @return bool true-绑定了模型，false-未绑定模型
 */
